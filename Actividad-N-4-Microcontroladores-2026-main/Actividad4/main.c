...
// Interrupci�n del Timer 0 en modo CTC. Se dispara cada 2ms (125 ticks a 16MHz/256).
...
// Centraliza toda la l�gica de tiempo real: parpadeo del LED, heartbeat alive,
// lectura de botones con debounce, y actualizaci�n de tiempos de brazos.
void On2Ms(void) {
	GPIOR0 &= ~_BV(GPIOR00);
...
// b) Seteo de callbacks entre m�dulos 
// c) Inicializa UART, puertos y timer
// d) Registro de botones 
// f) sei() habilita interrupciones solo cuando todo est� listo
int main(void) {
