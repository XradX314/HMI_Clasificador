...
// Extrae el destino m�s antiguo del buffer circular FIFO de la salida indicada
// Retorna -1 si el buffer est� vac�o, indicando que no hay cajas encoladas
...
// Eval�a el estado de los brazos y ejecuta las transiciones de la m�quina de estados
// Estados: 0=retra�do, 1=extendido, 2=esperando ACK de retracci�n
// Cuando el timer llega a 0 estando extendido, env�a comando de retraer al simulador
...
void CmdParser(uint8_t cmd, uint8_t* params, uint8_t len) {
	switch(cmd) {
		...
		// Busca el brazo que estaba esperando confirmaci�n (estado = 2) y lo libera.
		// Si hab�a una caja pendiente en esa salida, la procesa de inmediato:
		...
		// L�gica de encadenamiento de FIFOs:
		...
		// d) Si el FIFO est� vac�o, se guarda como pendiente hasta que llegue una caja.
        case 0x5E:  
